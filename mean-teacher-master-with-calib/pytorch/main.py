...
def main(context):
    ...
    train_loader, eval_loader,valid_indices= create_data_loaders(**dataset_config, args=args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.checkpoint_epochs and (epoch + 1) % args.checkpoint_epochs == 0:
            ...
            }, is_best, checkpoint_path, epoch + 1,valid_indices)
...
def create_data_loaders(train_transformation,
    ...
    indices = torch.randperm(len(labeled_idxs)+len(unlabeled_idxs)).numpy()
    test_dataset = torchvision.datasets.ImageFolder(evaldir, eval_transformation)
    valid_indices = indices[len(indices) - len(test_dataset):] if len(test_dataset) else None
    print(valid_indices)    


    return train_loader, eval_loader, valid_indices
...
def save_checkpoint(state, is_best, dirpath, epoch,valid_indices):
    filename = 'checkpoint.{}.ckpt'.format(epoch)
    checkpoint_path = os.path.join(dirpath, filename)
    #need to save validation set here too
    indices_filepath = os.path.join(dirpath, 'valid_indices.pth')
    torch.save(valid_indices, indices_filepath)
    best_val_path = os.path.join(dirpath, 'valid_indices_best.pth.tar')
    ...
    if is_best:
        ...
        shutil.copyfile(indices_filepath,best_val_path)
...
if __name__ == '__main__':
    ...
    #labeled data is at : /home/shreya/mean-teacher-master/pytorch/data-local/labels/bin/labels/chestdata/700_balanced_labels
